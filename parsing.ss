...
(define mersenne (list "2" "^" "31" "-" "1"))
(define logic (list "12" "==" "4" "+" "(" "2" "^" "4" ")" "-" "8"))
(define arit (list "2" "+" "3" "*" "(" "100" "/" "10" "^" "(" "8" "-" "4" ")" "+" "500" ")"))
...
(define (read-token)
  (let ((expr (readline "UofL")))
    (cond ((not (equal? expr "#exit"))
           (display (calculate (string-split expr)))
           (newline)
           (UofL))
          (else
           (display "Good bye"))
